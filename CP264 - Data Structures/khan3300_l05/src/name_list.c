...
void insert_name_list(sNAME_LIST *name_list, sNODE *node) {
	// If Student Node Data is Empty
	sNODE *temp = NULL;
	if (strcmp(node->name.first_name, "") == 0
			&& strcmp(node->name.last_name, "") == 0) {
		printf("\nName in new node does not exist\n");
		return;
	}

	// If Student Linked List is Empty
	...
	// Traverse List and Insert if Not in List
	else {
		temp = name_list->front;
		while (temp->next == NULL) {

			// If student already in list
			if (strcmp(temp->name.first_name, node->name.first_name) == 0
					&& strcmp(temp->name.last_name, node->name.last_name)
							== 0) {
				printf("\nName already exist in the list\n");
				free(node); // freeing the node since it isn't automatically freed
				return;
			}
			temp = temp->next;
		}

		// Insert in rear if not already in list
		temp->next = node;
		name_list->rear = node;
		name_list->count = name_list->count + 1;
	}
}
...
// Of course, the count field is set to ZERO for the same reason.
{
	...
	if (name_list != NULL) {    // Check for undefined/non-existent "name_list".
		...
		sprintf(msg[1], "%s",
				"Internal Error: List not Initialized because it does not exist!");
...
// including the header node.  It returns a count of the number of nodes freed, including the list header node.
{
	...
	if (name_list != NULL) {    // Check for undefined/non-existent "name_list".
		...
		if ((name_list->front != NULL) && (name_list->rear != NULL)
		...
		sprintf(msg[0], "%s%s", "In Function: ", pname);
		sprintf(msg[1], "%s",
				"Internal Error: List not freed because it does not exist!");
		report_message(msg, MSG_ERROR);
	}
	return (n_freed);
}
...
//==========================================
{
	...
	if (name_list != NULL) {    // Check for undefined/non-existent "name_list".
		...
		sprintf(msg[0], "%s%s", "In Function: ", pname);
		sprintf(msg[1], "%s",
				"Internal Error: List not printed because it does not exist!");
		report_message(msg, MSG_ERROR);
	}
	return (n_printed);
}
