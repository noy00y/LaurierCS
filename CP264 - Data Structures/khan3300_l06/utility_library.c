/*
 ...
 -------------------------------------
 Author:  Heider Ali and Rick Magnotta and Harriharan Sivakumar
 ID:      200676770
 Email:   heali@wlu.ca / rmagnotta@wlu.ca / siva6770@mylaurier.ca
 Version  2020-02-14
 Modified 2022-02-11
 -------------------------------------
 */
...
//       this is the routine that dumps/prints the stack contents.
{
	...
	for (p = p_tos; (p > p_bos); p -= STACK_VAL_LNG)
		printf("%s\n", p);
...
// have been previously GLOBALLY declared, but not initialized.
...
// NOTE: push() and pop() are not used in this utility routine in order to avoid an infinite calling loop.
{
	...
	// check overflow condition (p_tos>p_mxs)
	if ((p_tos + STACK_VAL_LNG) <= p_mxs) { // if not overflow
		p_tos += STACK_VAL_LNG; // move top pointer to the next position (should move to next string holder
		strcpy(p_tos, proc_name); // set the data in that position to be the proc_name
	}
	return;
}
...
//                                   The 10th byte contains the cNUL character.
{
	...
	if (size > 0) {
		for (i = 1; i < size; i++)
			*buff++ = fill; // By starting at (i=1) we loop (size-1) times AND the buff pointer points to the
