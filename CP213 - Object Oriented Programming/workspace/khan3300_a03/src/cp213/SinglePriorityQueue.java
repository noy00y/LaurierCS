...
public class SinglePriorityQueue<T extends Comparable<T>> extends SingleLink<T> {
	...
	// your code here
    	...
    	// data < queue data
    		...
    		node = new SingleNode<>(data, this.front);
    		this.front = node;
    	}
    	
    	// data >= queue data
    	else if (data.compareTo(this.rear.getData()) >= 0) {
    		node = new SingleNode<>(data, null);
	...
	// your code here
    	assert this.length > 0 : "cannot remove from a empty priority queue";
	...
	// your code here
    	...
    	// Higher
    	higher.length = middle;
    	higher.front = this.front;
    	higher.rear = prev;
    	
    	// Lower:
    	lower.length = this.length - middle;
    	lower.front = curr;
